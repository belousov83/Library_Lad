...
const commentFormComment = commentForm.comment;
const commentFormParentInput = commentForm.parent;
const commentFormSubmit = commentForm.commentSubmit;
const commentBookId = commentForm.getAttribute('data-book-id');
...
function replyComment() {
  ...
  commentFormComment.value = `${commentUsername}, `;
...
async function createComment(event) {
    ...
    try {
        const response = await fetch(`/books/${commentBookId}/comments/`, {
            method: 'POST',
            headers: {
                                ...
                                <li class="card border-0">
                                    <div class="row">
                                        <div class="col-md-10">
                                            <div class="card-body">
                                                <h6 class="card-title">
                                                    <a href="${comment.get_success_url}">${comment.visitor}</a>
                                                </h6>
                                                <p class="card-text">
                                                    ${comment.comment}
                                                </p>
                                                <a class="btn btn-sm btn-dark btn-reply" href="#commentForm" data-comment-id="${comment.id}" data-comment-username="${comment.visitor}">Ответить</a>
                                                <hr/>
